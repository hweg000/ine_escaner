...
function extractDocData(apiData) {
  ...
  const doc = idv?.ocr_result ?? idv?.document_data ?? idv?.extracted_data ?? idv?.data ?? idv?.result ?? idv?.fields ?? idv ?? {};
  console.log("📄 idv:", JSON.stringify(idv, null, 2));
  console.log("📋 doc:", JSON.stringify(doc, null, 2));
  const fullName = doc.full_name ?? doc.name ?? doc.fullName ?? (doc.first_name || doc.given_name || doc.firstName ? `${doc.first_name ?? doc.given_name ?? doc.firstName ?? ""} ${doc.last_name ?? doc.surname ?? doc.lastName ?? ""}`.trim() : "");
  return {
    full_name: fullName,
